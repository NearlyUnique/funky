...
";
        var @interface = new CodeFactory(source).Children<InterfaceDeclarationSyntax>();
        Assert.NotNull(@interface);

        var methods = SimpleSyntax.Members(@interface);
        var m = methods.Single();

        Assert.Equal("int ExampleProject.GoodMocks.IThing.Any(string text)", SourceCode.Signature(@interface.ToDisplayString(), m));
    }
    [Fact]
